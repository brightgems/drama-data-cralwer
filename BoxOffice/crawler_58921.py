...
def extract_boxoffice(html):
    ...
    for tr in trs:
        tds = tr.select("td")
        res = pt.search(str(tds[3]))
        id = res.group('id')
        yearrank = tds[0].getText(strip=True)
        histrank = tds[1].getText(strip=True)
        title = tds[2].getText(strip=True)
        img = tds[3].select('//img[@src]')
        audience = tds[4].getText(strip=True)
        screen = tds[4].getText(strip=True)
...
download_boxoffice_as_excel(2014,2016,'../data/boxoffice_2004_2016.xls')
